...
void DebugDrawer::build()
{
	...
	if (lineVertices.size() > 0 && isEnabled)
	{
		...
		std::list<VertexPair>::iterator lineVertIt = lineVertices.begin();
		std::list<VertexPair>::const_iterator lineVertEnd = lineVertices.end();
		for (lineVertIt; lineVertIt != lineVertEnd; ++lineVertIt)
		{
			manualObject->position(lineVertIt->first);
			manualObject->colour(lineVertIt->second);
		}

		std::list<int>::iterator lineIndIt = lineIndices.begin();
		std::list<int>::const_iterator lineIndEnd = lineIndices.end();
		for (lineIndIt; lineIndIt != lineIndEnd; ++lineIndIt)
			manualObject->index(*lineIndIt);
	}
	manualObject->end();
